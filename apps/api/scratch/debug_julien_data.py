...
async def debug_data():
    async with AsyncSessionLocal() as db:
        ...
        for o, u, offer in res.all():
            print(f"  Order {o.id} | Offer: {offer.name} | Price: {o.price_cents} | Status: {o.payment_status}")
            
        # Check installments for these orders
        res = await db.execute(select(Installment).where(Installment.tenant_id != None))
        print("\nAll Installments:")
