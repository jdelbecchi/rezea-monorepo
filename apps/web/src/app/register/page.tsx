...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      // Nettoyage des données : convertir les chaînes vides en null pour les champs optionnels
      // Cela évite les erreurs de validation côté serveur (ex: date de naissance vide)
  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-[#fbfcfd] flex items-center justify-center p-4 py-8">
       ...
       <div className={`bg-white/90 backdrop-blur-sm rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 p-6 md:p-10 w-full ${mode === "complet" ? "md:max-w-4xl" : "md:max-w-md"} relative z-10 transition-all duration-500`}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-slate-900 text-white p-3 rounded-xl font-medium text-sm hover:bg-slate-800 transition-all shadow-sm shadow-slate-200/50 disabled:opacity-50 active:scale-95 mt-4"
              >
            ...
            <p className="mt-8 text-center text-xs font-normal text-slate-400">
                Déjà inscrit ? <Link href="/" className="text-slate-600 hover:underline">Connexion</Link>
            </p>
        </div>
