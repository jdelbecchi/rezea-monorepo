...
function AdminSessionsContent() {
    ...
    const [warningModal, setWarningModal] = useState<{ show: boolean, users: { first_name: string, last_name: string }[] }>({ show: false, users: [] });
    ...
    const fetchSessions = useCallback(async () => {
        try {
            const now = new Date();
            const start = `${now.getFullYear() - 3}-01-01T00:00:00`;
            const end = `${now.getFullYear() + 2}-12-31T23:59:59`;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const apiError = err.response?.data?.detail || err.message || "Une erreur est survenue lors de la création.";
    ...
    const handleReactivateSession = async (session: Session) => {
        setConfirmModal({
            ...
            onConfirm: async () => {
                try {
                    const res = await api.reactivateSession(session.id);
                    await fetchSessions();
                    setConfirmModal(prev => ({ ...prev, show: false }));
                    if (res && res.failed_restorations && res.failed_restorations.length > 0) {
                        setWarningModal({
                            show: true,
                            users: res.failed_restorations
                        });
                    } else {
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row">
            ...
            <ConfirmModal
                ...
                title="⚠️ Inscriptions non rétablies"
                message={
                    <div className="space-y-2 text-left">
                        <p className="text-slate-600 text-sm">Les inscriptions des membres suivants n'ont pas pu être rétablies à cause d'un solde de crédits insuffisant :</p>
                        <ul className="list-disc pl-5 font-medium text-slate-800 text-sm">
                            {warningModal.users.map((u, i) => (
                                <li key={i}>{u.first_name} {u.last_name}</li>
                            ))}
                        </ul>
                        <p className="text-xs text-slate-400 mt-2">Leur inscription reste au statut "Séance annulée". Veuillez les contacter pour régulariser manuellement.</p>
                    </div>
                }
                type="warning"
                confirmLabel="D'accord"
                onConfirm={() => setWarningModal({ show: false, users: [] })}
            />
        </div>
    );
}
