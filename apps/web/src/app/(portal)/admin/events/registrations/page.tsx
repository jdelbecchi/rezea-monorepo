...
export default function AdminEventRegistrationsPage() {
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const apiError = err.response?.data?.detail || err.message || "Erreur lors de la création.";
    ...
    const handleEditSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const apiError = err.response?.data?.detail || err.message || "Erreur lors de la modification.";
            setModalError(typeof apiError === "string" ? apiError : JSON.stringify(apiError));
        } finally {
            setSaving(false);
        }
...
<style>
    ...
    const handleExport = async () => {
        try {
            const allData = await api.getAdminEventRegistrations({ include_deleted: true });
            const filtered = allData.filter((r) => {
                if (searchTerm) {
                    const q = searchTerm.toLowerCase();
                    if (!(r.user_name || "").toLowerCase().includes(q) &&
                        !(r.event_title || "").toLowerCase().includes(q)) return false;
                }
                if (filterStatuses.length > 0 && !filterStatuses.includes(r.status)) return false;
                if (filterPayments.length > 0 && !filterPayments.includes(r.payment_status)) return false;
                if (locationFilter.length > 0 && !(r.event_location && locationFilter.includes(r.event_location))) return false;
                
                if (dateRange.start && r.event_date && r.event_date < dateRange.start) return false;
                if (dateRange.end && r.event_date && r.event_date > dateRange.end) return false;
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row">
            ...
            <ConfirmModal
                ...
                message="Attention : cette action est irréversible. L'inscription sera définitivement supprimée."
                type="danger"
